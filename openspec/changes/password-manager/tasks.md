...
- [ ] 7.1 实现CategoryService类，包含CRUD方法
- [ ] 7.2 实现分类关联方法（关联密码条目到分类、取消关联）
- [ ] 7.3 实现分类过滤方法（按分类过滤密码条目）
- [ ] 7.4 实现分类管理界面CategoryManagementDialog
- [ ] 7.5 实现分类选择组件CategoryComboBox

## 8. 会话管理

- [ ] 8.1 实现SessionManager类，包含活动监控（鼠标、键盘事件监听）
- [ ] 8.2 实现超时检查定时器（每30秒检查一次）
- [ ] 8.3 实现锁定界面LockPanel（密码输入、解锁按钮）
- [ ] 8.4 实现手动锁定功能（锁定按钮、快捷键Ctrl+L）
- [ ] 8.5 实现敏感数据清除（内存中的密钥、剪贴板）
- [ ] 8.6 实现会话状态显示（剩余时间、锁定状态）

## 9. 剪贴板管理

- [ ] 9.1 实现ClipboardUtil类，包含copyToClipboard方法
- [ ] 9.2 实现自动清除定时器（可配置时间，默认30秒）
- [ ] 9.3 实现手动清除功能（清除按钮、快捷键）
- [ ] 9.4 实现剪贴板监控（检测内容变化、取消计时器）
- [ ] 9.5 实现复制历史记录（内存存储、不持久化）
- [ ] 9.6 实现复制确认对话框（可配置跳过）
